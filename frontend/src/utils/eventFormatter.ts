  ...
  switch (eventType) {
    ...
    case 'CRISIS_ESCALATION':
      return {
        ...
        eventType,
        category: 'crisis',
        title: String(payload.title || rawEvent.title || 'Crisis Anomaly Triggered'),
        description: String(payload.description || rawEvent.description || 'Emergency AI crisis escalation reported across multilateral networks.'),
        tick,
        timeDisplay,
        actor,
        severity: 'critical',
    ...
    case 'COUNTRY_DECISION':
      return {
        ...
        eventType,
        category: 'decision',
        title: String(payload.label || payload.action_name || rawEvent.title || 'Policy Action Committed'),
        description: String(payload.reasoning || payload.description || rawEvent.description || 'National authority dispatched autonomous response action.'),
        tick,
        timeDisplay,
        actor,
        severity: 'info',
    ...
    case 'COORDINATOR_PROPOSAL':
      return {
        ...
        title: String(payload.title || rawEvent.title || 'Multilateral Treaty Proposed'),
        description: String(payload.summary || payload.rationale || rawEvent.description || 'International Coordinator presented emergency mitigation framework for multilateral voting.'),
        tick,
        timeDisplay,
        actor,
        severity: 'warning',
    ...
    case 'NEGOTIATION_PASSED':
      return {
        ...
        eventType,
        category: 'negotiation',
        title: String(payload.headline || rawEvent.title || `Negotiation ${eventType.replace(/_/g, ' ')}`),
        description: String(payload.message || payload.outcome || rawEvent.description || 'Diplomatic voting round concluded among member nations.'),
        tick,
        timeDisplay,
        actor,
    ...
    case 'SCORE_UPDATE':
      return {
        ...
        eventType,
        category: 'system',
        title: `Governance Evaluation: Grade ${String(payload.score_grade || 'Calculated')}`,
        description: String(payload.performance_headline || rawEvent.description || `Overall Score: ${payload.overall_score ?? 0}/100.`),
        tick,
        timeDisplay,
        severity: 'success',
        ragCitation,
    ...
    default:
      ...
      return {
        ...
        eventType,
        category: 'general',
        title: String(payload.title || rawEvent.title || eventType.replace(/_/g, ' ')),
        description: String(payload.description || rawEvent.description || payload.message || payload.detail || JSON.stringify(payload)),
        tick,
        timeDisplay,
        actor,
        severity: 'info',
